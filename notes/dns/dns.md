...
* Authoritative DNS Server — DNS-сервер, который содержит актуальную информацию о записях своей DNS-зоны и отвечает за неё.

* SOA (Start of Authority) — DNS-запись, которая содержит основные служебные параметры DNS-зоны и указывает основной (primary/master) DNS-сервер.

* AXFR — тип DNS-запроса для полной передачи DNS-зоны, позволяющий получить все записи зоны при разрешённом zone transfer.

* IXFR — тип DNS-запроса для передачи только изменений DNS-зоны.

* PTR — DNS-запись для reverse DNS, позволяющая получить hostname по IP-адресу.

* NS (Name Server) — DNS-запись, указывающая authoritative DNS-серверы, обслуживающие конкретную DNS-зону.
...
**GeoDNS (Geographic DNS)** — технология, при которой DNS-сервер возвращает разные IP-адреса в зависимости от **географического расположения клиента**.
...
DNS
      ↓
203.0.113.10   ← сервер в Европе

client из США
      ↓
DNS
      ↓
203.0.113.20   ← сервер в США

client из Азии
      ↓
DNS
      ↓
203.0.113.30   ← сервер в Азии
```

### Как GeoDNS узнаёт расположение клиента

При обычном DNS-запросе авторитетный сервер обычно видит IP **DNS-резолвера**, а не конечного пользователя:
...
Поэтому используется **ECS (EDNS Client Subnet)**.

Резолвер может передать авторитетному DNS **префикс IP клиента**, например:
...
Авторитетный DNS получает не полный IP, а только подсеть клиента.

После этого GeoDNS определяет регион:
...
DNS Resolver
   ↓
ECS: client subnet /24
   ↓
Authoritative DNS
   ↓
GeoDNS определяет регион
   ↓
возвращает подходящий IP
```

**Запомнить:**

> **GeoDNS может отдавать разные IP для одного домена в зависимости от местоположения клиента. ECS помогает авторитетному DNS определить местоположение клиента, передавая ему префикс его IP-адреса, например `/24`, вместо полного IP.**
